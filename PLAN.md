...
- **Editor**: headings, body, bold/italic/underline/strike, inline code, bullet and numbered lists, quote, divider, image insert, 600ms debounced autosave with a Saved/Saving indicator, `Ctrl+Enter` to save now, `Ctrl+Shift+8` for a bullet list, paste and drop of images, and the floating Smaller/Larger/Remove toolbar on a selected image. Colour and highlight sit behind a swatch row, with `Ctrl+Shift+H` for the first tint and a one-step undo of the editor's own, since the wrappers are placed by hand - see DECISIONS 2026-09-07. The formatting buttons show the caret's own formatting - heading level, body, list, quote, code and the four inline marks - read off the document on every selection change rather than remembered; see DECISIONS 2026-09-07.
- **Search** across the note bodies, not only the title and the preview: read on the first search of the session in one batched call ([readNotes](src/main/storage.ts)), cached against each note's `edited` stamp in [useSearchText.ts](src/renderer/src/lib/useSearchText.ts), and shown as the matched line on the card. 24.6ms for 130 notes, and nothing measurable per keystroke after that. A search of five characters or more is also tried with one inflectional ending removed, so a form finds its base word, and thirteen listed pairs of vowel-changing stems are followed both ways so `böcker` finds `bok` - see DECISIONS 2026-09-07.
- **Ctrl+K** in [launcher.ts](src/renderer/src/lib/launcher.ts) and [Launcher.tsx](src/renderer/src/app/Launcher.tsx): type a folder's name and the first row makes a note in it, on the template that folder's own notes are tagged with. Titles and folder names only - the bodies are Ctrl+Shift+F's job, so the launcher answers between keystrokes. Same key and same keys as Helm's. See DECISIONS 2026-09-09.
- **Columns** in [notes.ts](src/renderer/src/lib/notes.ts) and the editor: a row of two or three, from the toolbar or `/columns`, with Tab between them, a strip on the row to change the count or take it apart, a flag gutter per column, and a load-time repair that believes the cells rather than the count. A row's outer edges are reachable: a click in the margin beside one opens a line there, ArrowUp and ArrowDown leave the columns, and no delete key can merge the line below a row into its last column. Laid out by the stylesheet from `data-cols` and `data-col` alone, so a sticky window shows the same row with no code running over it - see DECISIONS 2026-09-07.
